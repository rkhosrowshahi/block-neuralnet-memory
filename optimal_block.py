...
if __name__ == "__main__":
    ...
    os.makedirs(f"{res_path}/codebooks", exist_ok=True)
    hist_file_path = res_path + "/hist_table.csv"
    df = None
    if os.path.exists(hist_file_path):
        ...
        df.to_csv(hist_file_path, index=False)
    ...
    df = None
    if os.path.exists(f"{res_path}/paretofront.csv"):

        df = pd.read_csv(f"{res_path}/paretofront.csv")
    else:
        res = minimize(
            problem,
            algorithm,
        ...
        df.to_csv(f"{res_path}/paretofront.csv", index=False)
    ...
    plt.savefig(f"{res_path }/paretofront.pdf")
